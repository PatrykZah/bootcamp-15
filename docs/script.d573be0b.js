...
function () {
  ...
  _createClass(Kanban, [{
    ...
    value: function newboard(data) {
      ...
      this.board.querySelector('.btn-create-column').addEventListener('click', function () {
        // add column
        _this.oPrompt('o-column-template', {}, _this.newcolumn.bind(_this));
      });
      this.board.querySelector('.btn-edit2').addEventListener('click', function () {
        // edit board
        _this.oPrompt('o-board-template', _this.data, _this.data);
      });
      this.board.querySelector('.btn-del').addEventListener('click', function () {
        // remove board
        _this.board.remove(); // todo remove from prototype.board

      });
      this.PropertyDescriptorsMerge(this.data, {
        this: this
      }, this.reference(this.board, {
        id: '.id'
      }), this.reference(this.board.querySelector('.title'), {
        title: '.textContent'
      }), this.reference(this.board.querySelector('.board-nav'), {
        rgb: '.style["background-color"]'
      }), {
        get color() {
          return this.this.rgbTohex(this.rgb);
        },

        set color(value) {
          this.rgb = this.this.rgbTohex(value);
        }
    ...
    value: function newcolumn(data) {
      ...
      var elm = this.generateTemplate({
        name: 'column-template',
        tmpl_data: data
      });
      this.board.querySelector(".columns-container").appendChild(elm); // this.columns[data.id] = data
      ...
      this.PropertyDescriptorsMerge(this.columns[data.id].data, {
        ...
        id: '.id'
      }), this.reference(elm.querySelector('.column-title'), {
        title: '.textContent'
      }), this.reference(elm.querySelector('.column-nav'), {
        rgb: '.style["background-color"]'
      }), {
        get color() {
          return this.this.rgbTohex(this.rgb);
      ...
      elm.querySelector('.btn-add').addEventListener('click', function () {
        _this2.oPrompt('o-card-template', {
          column: elm
        }, _this2.newcard.bind(_this2));
      }); // event edit column

      elm.querySelector('.btn-edit2').addEventListener('click', function () {
        _this2.oPrompt('o-column-template', _this2.columns[data.id].data, _this2.columns[data.id].data);
      }); // event remove column

      elm.querySelector('.btn-del').addEventListener('click', function () {
        // log(elm,this,data)
        delete _this2.columns[data.id];
        elm.remove();
      });
      return elm;
    }
    ...
    value: function newcard(data) {
      ...
      var elm = data.node = this.generateTemplate({
        name: 'card-template',
        tmpl_data: data
      });
      ...
      this.PropertyDescriptorsMerge(this.cards[data.id].data, {
        ...
        id: '.id'
      }), this.reference(elm.querySelector('.card-nav'), {
        rgb: '.style["background-color"]'
      }), this.reference(elm.querySelector('.card-title'), {
        title: '.textContent'
      }), this.reference(elm.querySelector('.card-text'), {
        content: '.textContent'
      }), {
        get color() {
          return this.this.rgbTohex(this.rgb);
      ...
      elm.querySelector('.btn-edit').addEventListener('click', function () {
        console.log(_this3, _this3.cards[data.id].data); // eslint-disable-line

        _this3.oPrompt('o-card-template', _this3.cards[data.id].data, _this3.cards[data.id].data);
      }); // event remove card

      elm.querySelector('.btn-del').addEventListener('click', function () {
        delete _this3.cards[data.id];
        elm.remove();
      });
    }
  }, {
    ...
    key: "oPrompt",
    value: function oPrompt(tmpl, presetdata, callback) {
      // tmpl,presetdata{},callback
      var overlay = document.createElement('div');
      overlay.id = 'overlay';
      overlay.appendChild(this.generateTemplate({
        name: tmpl,
      ...
      overlay.children[0].addEventListener('click', function () {
        event.stopPropagation();
      });
      overlay.querySelector('button').addEventListener('click', function () {
        var items = overlay.querySelectorAll('input,textarea');
        var obj = presetdata;
...
board = new Kanban({
  title: 'preset board created from js',
  color: '#696'
});
board.newcolumn({
  title: 'js column'
...
board.newcard({
  title: 'js card',
  content: 'you can edit me or... delete me',
  column: board.columns[Object.keys(board.columns)[0]].node
}); // a bit ugly way but whatever

board.newcolumn({
  title: 'js column2'
});
},{"sortablejs":"tZHd","mustache":"gZU7"}]},{},["L4bL"], null)
