...
function () {
  function Kanban(data) {
    ...
    this.board = this.generateTemplate({
      name: 'board-template',
      tmpl_data: data
    });
    document.children[0].children[1].appendChild(this.board);
    this.PropertyDescriptorsMerge(this.data, this.reference(this.board, {
      id: 'id'
    }), this.reference(this.board.querySelector('.title'), {
      title: 'textContent'
    }), this.reference(getComputedStyle(this.board.querySelector('.board-nav')), {
  ...
  _createClass(Kanban, [{
    ...
    key: "reference",
    value: function reference(parent, data) {
      // parent node/string,{*data id*:*where it is*} //// bypass primitive no reference :)
      // reference(Kanban.prototype.boards[0].board,{content:'.textContent',color:'.style.backgroundColor'})
      var Obj = {};
      var proto = Object.getPrototypeOf(this);

      if (!proto.referenceCounter) {
        proto.referenceCounter = 0;
      }

      var node_id = '_node' + proto.referenceCounter++;
      ...
      for (var id in data) {
        Object.defineProperty(Obj, id, {
          get: new Function("return this['".concat(node_id, "']['").concat(data[id], "']")),
          set: new Function('value', "this['".concat(node_id, "']['").concat(data[id], "'] = value"))
        });
      }
    ...
    key: "PropertyDescriptorsMerge",
    value: function PropertyDescriptorsMerge(target) {
    ...
    value: function generateTemplate(data) {
      // template id , data, node target
      if (!(_typeof(data) === 'object' && data.name && data.tmpl_data)) {
        log(data);
        throw new Error('Kanban.generateTemplate expect data.name:Template #id,data.tmpl_data:{...}');
      }

      var template = document.getElementById(data.name).innerHTML;
      Mustache.parse(template);
      return new DOMParser().parseFromString(Mustache.render(template, data.tmpl_data), 'text/html').body.childNodes[0];
    }
  }, {
    key: "o_prompt",
    value: function o_prompt(tmpl, presetdata, zero) {
      // tmpl,presetdata{},[boolean - force zero data]
      console.log(arguments);
      var overlay = this.generateTemplate({
        name: tmpl,
        tmpl_data: presetdata
      });
